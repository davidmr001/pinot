...
public class DefaultGroupKeyGenerator implements GroupKeyGenerator {
  ...
  private final int[] _reusableDecodeDictIdBuffer;
  ...
  DefaultGroupKeyGenerator(DataFetcher dataFetcher, String[] groupByColumns, int maxNumGroupKeys) {
    ...
    _reusableDecodeDictIdBuffer = new int[_numGroupByColumns];
      ...
      if (_isSingleValueGroupByColumn[i]) {
        ...
        for (int j = 0; j < numMultiValues; j++) {
          for (int k = 0; k < originalSize; k++) {
            ...
            groupByKeys.set(idx, (groupByKeys.get(idx) * _cardinalities[i]) + _reusableMultiValDictIdBuffer[j]);
          }
        }
      }
    }
  ...
  private String rawKeyToStringGroupKey(int rawKey) {
    ...
    if (_numGroupByColumns == 1) {
      ...
      for (int i = _numGroupByColumns - 1; i >= 0; i--) {
        ...
        _reusableDecodeDictIdBuffer[i] = rawKey % cardinality;
        rawKey /= cardinality;
      }
      return buildStringGroupKey();
    }
  }
  ...
  private String rawKeyToStringGroupKey(long rawKey) {
    ...
    for (int i = _numGroupByColumns - 1; i >= 0; i--) {
      int cardinality = _cardinalities[i];
      _reusableDecodeDictIdBuffer[i] = (int) (rawKey % cardinality);
      rawKey /= cardinality;
    }
    return buildStringGroupKey();
  }

  private String buildStringGroupKey() {
    StringBuilder builder = new StringBuilder();
    for (int i = 0; i < _numGroupByColumns; i++) {
      String key = _dictionaries[i].get(_reusableDecodeDictIdBuffer[i]).toString();
      if (i > 0) {
